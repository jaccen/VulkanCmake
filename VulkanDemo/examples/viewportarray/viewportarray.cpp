...
#define MULVIEW_COL    4  //多少列
#define MULVIEW_ROW    4  //多少行
#define MULVIEW_SUM    MULVIEW_COL * MULVIEW_ROW
...
public:
	...
	struct UBOGS {
		glm::mat4 projection[MULVIEW_SUM];
		glm::mat4 modelview[MULVIEW_SUM];
	...
	{
		title = "Viewport arrays: " + std::to_string(MULVIEW_SUM);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			VkViewport viewports[MULVIEW_SUM];
			VkRect2D scissorRects[MULVIEW_SUM];

			for (int m = 0; m < MULVIEW_ROW; ++m) {
				for (int n = 0; n < MULVIEW_COL; ++n) {
					viewports[m * MULVIEW_COL + n] = { (float)(width * n )/(float)MULVIEW_COL, (float)(height* m) / (float)MULVIEW_ROW, (float)(width) / (float)MULVIEW_COL, (float)(height) / (float)MULVIEW_ROW, 0.0f, 1.0f};
					scissorRects[m * MULVIEW_COL + n] = vks::initializers::rect2D(width / MULVIEW_COL, height/ MULVIEW_ROW, (float)(width * n) / (float)MULVIEW_COL, (float)(height * m) / (float)MULVIEW_ROW);

				}
			}
	
			vkCmdSetViewport(drawCmdBuffers[i], 0, MULVIEW_SUM, viewports);

		
			vkCmdSetScissor(drawCmdBuffers[i], 0, MULVIEW_SUM, scissorRects);
	...
	void loadAssets()
	{
		scene.loadFromFile(getAssetPath() + "models/reflection_scene.gltf", vulkanDevice, queue, vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::PreMultiplyVertexColors | vkglTF::FileLoadingFlags::FlipY);
	}
	...
	void preparePipelines()
	{
		...
		VkPipelineViewportStateCreateInfo viewportState = vks::initializers::pipelineViewportStateCreateInfo(MULVIEW_SUM, MULVIEW_SUM, 0);
	...
	void updateUniformBuffers()
	{
		...
		for (int i = 0; i < MULVIEW_SUM; ++i) {
			uboGS.projection[i] = glm::frustum(left, right, bottom, top, zNear, zFar);
			uboGS.modelview[i] = rotM * transM;
		}
